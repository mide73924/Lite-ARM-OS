...
1. Install the ARM cross-compiler for Cortex-M:

```bash
sudo dnf install arm-none-eabi-gcc-cs arm-none-eabi-newlib
```

- `arm-none-eabi-gcc-cs`: Cortex-M compiler
- `arm-none-eabi-newlib`: standard library for bare-metal targets
...
- Resulting binary: `arm-softmmu/qemu-system-arm`
- `dtc/libfdt/libfdt.so.1` is required at runtime

---

### 4. Set `LD_LIBRARY_PATH`

QEMU needs to find the correct `libfdt` from the dtc submodule:

```bash
export LD_LIBRARY_PATH=/full/path/to/qemu_stm32/dtc/libfdt:$LD_LIBRARY_PATH
```

- Replace `/full/path/to/qemu_stm32` with your actual folder path.
- Keep this set in every terminal session running QEMU.

---

### 5. Build mini-arm-os Example

1. Navigate to an example folder:

```bash
cd /full/path/to/mini-arm-os/00-HelloWorld
```

2. Build the example for STM32-P103:

```bash
make PLAT=p103
```

- Produces `hello.elf`, `hello.bin`, and `hello.list`.
...
```bash
/full/path/to/qemu_stm32/arm-softmmu/qemu-system-arm \
  -M stm32-p103 \
  -kernel hello.bin
```

- Output appears via the **emulated UART** in the terminal.
- Example output for `00-HelloWorld`:
...
### 7. Workflow for Subsequent Examples

1. Move to the next numbered folder, e.g.:
...
2. Run in QEMU as above:

```bash
../../qemu_stm32/arm-softmmu/qemu-system-arm -M stm32-p103 -kernel hello.bin
```

3. Repeat incrementally through all examples.

---

### 8. Notes / Tips

- Each numbered folder in `mini-arm-os` is **independent**. Build and run them individually.
- Keep `LD_LIBRARY_PATH` pointing to `dtc/libfdt` while running QEMU to avoid symbol errors.
- Modify example `.c` files to experiment — changes appear immediately after rebuild.

---

This setup lets you **compile, run, and experiment with a minimal ARM OS safely inside QEMU** on RHEL 9 without touching the host system.
